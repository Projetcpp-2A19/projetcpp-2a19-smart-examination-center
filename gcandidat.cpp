...
gcandidat::gcandidat(QWidget *parent)
    : QMainWindow(parent), ui(new Ui::gcandidat)
...
bool gcandidat::verifierSaisie(QString code, QString prenom, int cin, QString adresse, QString nom, int numTel)
{
...
void gcandidat::on_AjButton_5_clicked()
{
...
void gcandidat::afficherCandidats()
{
    QSqlQuery query;
...
void gcandidat::on_btnSupprimer_clicked()
{
    QString code = ui->lineCode->text();
...
void gcandidat::on_ModButton_5_clicked()
{
    QString code = ui->lineCode->text();
    QString prenom = ui->linePrenom->text();
    int cin = ui->lineCIN->text().toInt();
...
void gcandidat::on_lineCode_textChanged(const QString &code)
{
...
void gcandidat::on_btnTrierNiveau_clicked()
{
...
void gcandidat::on_btnRechercher_clicked()
{
...
void gcandidat::on_btnStats_clicked()
{
...
void gcandidat::on_btnpdf_clicked()
{
...
void gcandidat::on_btnConvocation_clicked()
{

}
QImage gcandidat::genererQRCodeImage(const QString& data, int pixelParModule) {
...
bool gcandidat::genererConvocationPDF(const QString& codeCandidat)
{
...
void gcandidat::on_btnAfficherConvocation_clicked()
{
...
void gcandidat::readSerialData() {
    int quantite = 20;
    qDebug() << "Quantité fixe (Qt):" << quantite;  // Affiche la quantité dans la console (debug)

    // 2. Définir un ID fixe pour l'équipement (par exemple, équipement n°8)
    int equipmentId = 8;
    qDebug() << "ID Equipement fixe:" << equipmentId;  // Affiche l'ID sélectionné pour mise à jour

    // 3. Définir une quantité fictive existante dans la base de données (comme une valeur lue depuis DB)
    int quantityFromDB = 5;
    qDebug() << "Quantité fixe (DB simulée):" << quantityFromDB;  // Affiche cette quantité pour vérification

    // 4. Calculer la somme des deux quantités (celle venant d'Arduino + celle existant dans la base)
    int somme = quantite + quantityFromDB;
    qDebug() << "Somme :" << somme;  // Affiche le total à insérer dans la base

    // 5. Mettre à jour la base de données avec la nouvelle quantité
    QSqlQuery updateQuery;  // Créer une requête SQL
    updateQuery.prepare("UPDATE EQUIPEMENTS SET QUANTITE__EQUIPEMENT = :newQuantite WHERE ID_EQUIPEMENT = :id");
    // Préparer une requête paramétrée pour éviter les injections SQL

    updateQuery.bindValue(":newQuantite", somme);  // Associer la valeur totale calculée au paramètre :newQuantite
    updateQuery.bindValue(":id", equipmentId);      // Associer l'ID fixe de l'équipement au paramètre :id

    // Exécuter la requête SQL
    if (updateQuery.exec()) {
        qDebug() << "✅ Quantité mise à jour avec succès dans la base.";  // Affiche un message de succès
    } else {
        qDebug() << "❌ Erreur lors du UPDATE:" << updateQuery.lastError();  // Affiche un message d'erreur si échec
        return;  // Sort de la fonction en cas d'erreur
    }

    // 6. Envoyer la somme calculée vers l’Arduino via le port série
    QByteArray dataToSend = "SOMME:" + QByteArray::number(somme) + "\n";  // Crée le message à envoyer (ex: "SOMME:24")
    A.write_to_arduino(dataToSend);  // Envoie le message à l’Arduino à travers la communication série
    qDebug() << "📤 Envoyé à Arduino:" << dataToSend;  // Affiche dans la console ce qui a été envoyé à l'Arduino
}
