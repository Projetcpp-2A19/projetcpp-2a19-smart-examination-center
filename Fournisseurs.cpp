...
bool Fournisseur::ajouterFourn() {
    // Vérifie que la base est ouverte
    if (!QSqlDatabase::database().isOpen()) {
        qDebug() << "Connexion à la base de données non ouverte.";
        return false;
    }
    QSqlQuery query;
    // Première requête pour obtenir le prochain ID disponible
    query.exec("SELECT NVL(MAX(TO_NUMBER(ID_FOURNISSEUR)), 0) + 1 FROM FOURNISSEURS");
    int nextId = 1;
    if (query.next()) {
        nextId = query.value(0).toInt();
    }
    qDebug() << "Valeurs insérées :"
             << "ID:" << nextId
             << "Nom:" << nom_fourn
             << "Email:" << email
             << "NumTel:" << numTel
    ...
    // Deuxième requête pour l'insertion
    query.prepare("INSERT INTO FOURNISSEURS (ID_FOURNISSEUR, NOM_FOURNISSEUR, EMAIL_FOURNISSEUR, NUMTEL_FOURNISSEUR, ADRESSE_FOURNISSEUR, TYPESERVICE_FOURNISSEUR) "
                  "VALUES (:id, :nom_fourn, :email, :numTel, :adresse, :typeService)");
...
QSqlQueryModel* Fournisseur::afficherFourn() {
    QSqlQueryModel* model_Fourn = new QSqlQueryModel();
    ...
    // Exécuter la requête
    model_Fourn->setQuery("SELECT ID_FOURNISSEUR, NOM_FOURNISSEUR, NUMTEL_FOURNISSEUR, EMAIL_FOURNISSEUR, ADRESSE_FOURNISSEUR, TYPESERVICE_FOURNISSEUR FROM FOURNISSEURS");

    // Vérifier si la requête SQL a bien fonctionné
    if (model_Fourn->lastError().isValid()) {
        qDebug() << "❌ ERREUR SQL : " << model_Fourn->lastError().text();
        return nullptr;
    }
    ...
    model_Fourn->setHeaderData(0, Qt::Horizontal, QObject::tr("ID"));
    model_Fourn->setHeaderData(1, Qt::Horizontal, QObject::tr("Nom"));
    model_Fourn->setHeaderData(2, Qt::Horizontal, QObject::tr("Numéro"));
    model_Fourn->setHeaderData(3, Qt::Horizontal, QObject::tr("Email"));
    model_Fourn->setHeaderData(4, Qt::Horizontal, QObject::tr("Adresse"));
    model_Fourn->setHeaderData(5, Qt::Horizontal, QObject::tr("Service"));
...
bool Fournisseur::supprimerFourn(int id) {
    QSqlQuery query;
...
bool Fournisseur::modifierFourn( QString id) {
    QSqlQuery query;
